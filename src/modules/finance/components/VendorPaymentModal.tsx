...
const VendorPaymentModal: React.FC<VendorPaymentModalProps> = ({ vendorIds, bookingId, onClose }) => {
  ...
  const { updateVendorBalance } = useOperationalData();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Sync payment to OperationalDataContext so FinanceTMSBridge reflects updated vendor balances
    const splitAmt = vendorIds.length > 0 ? numAmount / vendorIds.length : numAmount;
    vendorIds.forEach(vid => updateVendorBalance(vid, -splitAmt));
    showToast({ type: 'success', title: 'Payment Recorded', message: `₹${formatINR(numAmount)} payment recorded successfully.` });
    onClose();
  };
