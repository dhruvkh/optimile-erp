...
const InvoiceList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    showToast({
      type: 'success',
      title: `${pending.length} PTL invoice${pending.length > 1 ? 's' : ''} imported`,
      message: 'Draft invoices created from delivered PTL dockets.',
    });
  ...
  const handleSend = (id: string, e: React.MouseEvent) => {
    ...
    showToast({ type: 'success', title: 'Invoice Sent', message: 'Invoice marked as sent.' });
  };
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
    ...
    showToast({ type: 'success', title: 'Invoice Deleted', message: 'Invoice has been removed.' });
  };
