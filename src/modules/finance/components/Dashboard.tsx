...
const CostPerMileChart: React.FC<{ vehicles: Vehicle[], expenses: any[] }> = ({ vehicles, expenses }) => {
  ...
  const handleClick = (data: any) => {
    if (data && data.activePayload && data.activePayload.length > 0) {
      ...
      navigate('/finance/fleet-ledger', { state: { searchQuery: vehicleReg } });
    }
  };
      ...
      <div className="h-64 cursor-pointer">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart layout="vertical" data={data} margin={{ left: 20 }} onClick={handleClick}>
            ...
            <Tooltip formatter={(val: any) => `₹${Number(val).toFixed(2)} /km`} />
  ...
  const data = useMemo(() => {
    ...
    return Array.from({ length: 6 }, (_, i) => {
      ...
      const revenue = invoices
        .filter(inv => { const dd = new Date(inv.date); return dd.getMonth() === mo && dd.getFullYear() === yr; })
        .reduce((s: number, inv: any) => s + inv.amount, 0);
      const cost = expenses
        .filter(exp => { const dd = new Date(exp.date); return dd.getMonth() === mo && dd.getFullYear() === yr; })
        .reduce((s: number, exp: any) => s + exp.amount, 0);
      return { name: MONTHS[mo], revenue, cost };
    });
  }, [invoices, expenses]);
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data}>
            ...
            <Tooltip formatter={(val: any) => formatCompactINR(Number(val))} />
            <Legend />
...
const CustomerProfitabilityChart: React.FC<{ customers: any[], invoices: any[], bookings: any[] }> = ({ customers, invoices, bookings }) => {
  const data = useMemo(() => {
    return customers.slice(0, 15).map(c => {
      const rev = invoices.filter((i: any) => i.customerId === c.id).reduce((acc: number, cur: any) => acc + cur.amount, 0);
      // Use actual trip expense data from bookings rather than simulating a random cost ratio
      const cost = bookings.filter((b: any) => b.customerId === c.id).reduce((acc: number, b: any) => acc + (b.expense || 0), 0);
      return { name: c.name.split(' ')[0], revenue: rev, cost };
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data}>
            ...
            <YAxis hide />
            <Tooltip cursor={{ fill: 'transparent' }} formatter={(val: any) => formatCompactINR(Number(val))} />
...
const LaneIntelligence: React.FC<{ bookings: Booking[] }> = ({ bookings }) => {
  ...
  const breakdownData = useMemo(() => {
    ...
    // Estimate breakdown from lane expense using industry-standard cost ratios
    const total = selectedLane.expense || 1;
    return [
      { name: 'Fuel', value: Math.round(total * 0.50), fill: '#ef4444' },
      { name: 'Driver', value: Math.round(total * 0.22), fill: '#3b82f6' },
      { name: 'Tolls', value: Math.round(total * 0.18), fill: '#f59e0b' },
      { name: 'Maint', value: Math.round(total * 0.10), fill: '#10b981' }
    ];
  }, [selectedLane]);
  ...
  return (
    <div className="col-span-1 md:col-span-2 xl:col-span-2 bg-white p-6 rounded-lg shadow-sm border border-gray-100 relative">
      ...
      <div className="h-64 cursor-pointer">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={lanes} layout="vertical" onClick={(data: any) => data?.activePayload && setSelectedLane(data.activePayload[0].payload)}>
            <CartesianGrid strokeDasharray="3 3" horizontal={false} />
            <XAxis type="number" hide />
...
const ApprovalQueue: React.FC = () => {
  ...
  const pendingAdvances = getAllPendingAdvances();

  const handleApprove = (tripId: string, expenseId: string, amount: number) => {
    ...
    showToast({ type: 'success', title: 'Advance Approved', message: `₹${amount.toLocaleString('en-IN')} advance approved and sent to Finance.` });
  };

  const handleReject = (tripId: string, expenseId: string) => {
    updateTripExpense(tripId, expenseId, { status: 'Rejected', rejectionReason: 'Rejected via Finance Dashboard' });
    showToast({ type: 'warning', title: 'Advance Rejected', message: 'Advance has been rejected.' });
  };

  return (
    <div className="col-span-1 md:col-span-2 xl:col-span-4 bg-white p-6 rounded-lg shadow-sm border border-gray-100">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-lg font-semibold text-gray-800 flex items-center">
            <span className="material-icons text-blue-500 mr-2">verified_user</span>
            Pending Trip Advance Approvals
          </h3>
          <p className="text-xs text-gray-500 mt-1">Approve or reject pre-trip advance requests from Operations.</p>
        </div>
        <span className="text-sm font-bold text-gray-900 bg-blue-50 px-3 py-1 border border-blue-200 rounded">
      ...
      {pendingAdvances.length === 0 ? (
        <div className="text-center py-10 text-gray-400">
          <p className="text-sm font-medium">No pending advance approvals</p>
          <p className="text-xs mt-1">Advance requests from dispatched trips will appear here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {pendingAdvances.slice(0, 8).map(adv => (
            <div key={adv.id} className="p-4 rounded-lg border border-primary bg-blue-50/30 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-gray-500 bg-white px-2 py-0.5 rounded border">
                  {ADVANCE_TYPE_LABELS[adv.advanceType || ''] || adv.category}
                </span>
                <span className="text-xs text-gray-400">{adv.date}</span>
              </div>
              <p className="text-lg font-bold text-gray-900 my-2">{formatINR(adv.amount)}</p>
              <p className="text-xs text-gray-600 mb-1">Trip: <span className="font-medium">{adv.trip.id}</span></p>
              <p className="text-xs text-gray-500 mb-4 truncate">{adv.trip.clientName} · {adv.trip.origin} → {adv.trip.destination}</p>
              <div className="border-t pt-3 flex space-x-2">
                <button
                  onClick={() => handleApprove(adv.tripId, adv.id, adv.amount)}
                  className="flex-1 bg-green-600 text-white text-xs font-bold py-1.5 rounded hover:bg-green-700 transition"
                >
                  Approve
                </button>
                <button
                  onClick={() => handleReject(adv.tripId, adv.id)}
                  className="flex-1 bg-white border border-red-200 text-red-600 text-xs font-bold py-1.5 rounded hover:bg-red-50 transition"
                >
                  Reject
                </button>
              </div>
...
const Dashboard: React.FC = () => {
  ...
  // Sparkline: last 7 months of invoice revenue (normalised to 0-100 scale)
  const sparkRaw = useMemo(() => {
    const now = new Date();
    return Array.from({ length: 7 }, (_, i) => {
      const d = new Date(now.getFullYear(), now.getMonth() - 6 + i, 1);
      return state.invoices
        .filter(inv => { const dd = new Date(inv.date); return dd.getMonth() === d.getMonth() && dd.getFullYear() === d.getFullYear(); })
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 pb-12">
              ...
              <div className="flex h-4 mt-4 w-full rounded overflow-hidden">
                {receivablesData.map((d, i) => (
                  <div
                    ...
                    onClick={() => navigate('/finance/customers', { state: { minOverdueDays: d.minDays, statusFilter: 'overdue' } })}
                  />
                ))}
              </div>
            }
      ...
      {!isOpsManager && (
        <>
          ...
          <div className="col-span-1 md:col-span-2 xl:col-span-1">
            <CustomerProfitabilityChart customers={state.customers} invoices={state.invoices} bookings={state.bookings} />
          </div>
