...
const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  const staticActions: CommandItem[] = [
    { type: 'nav', label: 'Go to Dashboard', icon: 'dashboard', path: '/finance/dashboard' },
    { type: 'nav', label: 'Create New Invoice', icon: 'add_circle', path: '/finance/invoices/create' },
    { type: 'nav', label: 'View Customers', icon: 'people', path: '/finance/customers' },
    { type: 'nav', label: 'View Fleet', icon: 'directions_car', path: '/finance/fleet-ledger' },
    { type: 'nav', label: 'Settings', icon: 'settings', path: '/finance/settings' },
  ];
  ...
  const handleSelect = (item: CommandItem) => {
    ...
    if (item.type === 'nav' && item.path) {
      ...
      navigate('/finance/customers', { state: { searchQuery: item.label } });
    } else if (item.type === 'invoice') {
      navigate('/finance/invoices', { state: { searchQuery: item.label } });
    }
  };
